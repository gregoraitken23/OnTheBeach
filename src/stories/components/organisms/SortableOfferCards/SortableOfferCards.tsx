...
const SortableInfoCards: React.FC<Props> = ({ data }) => {
  ...
  const handleSort = (option: SortOptions) => {
    ...
    switch (option) {
      case 'alphabetical':
        sortedArray = [...data].sort((a, b) => a.hotelName.localeCompare(b.hotelName))
        break
      case 'price':
  ...
  return (
      ...
      <aside className="sidebar">
        <SortButton buttonText='sort' secondaryBoldText='alphabetically' onClick={() => { handleSort('alphabetical') }} icon={<Alphabetical />}/>
        <SortButton buttonText='sort by' secondaryBoldText='price' onClick={() => { handleSort('price') }} icon={<Pound />}/>
        <SortButton buttonText='sort by' secondaryBoldText='star rating' onClick={() => { handleSort('star') }} icon={<Star />}/>
      </aside>
